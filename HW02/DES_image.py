#Homework Number: HW 02 problem 2
#Name: Peiyuan Li
#ECN Login: li1867
...
def DES(input, output, encrypt):
    ...
    for header in header_list:
        ...
        #read header size of BitVector
        bv.read_bits_from_file(len(header) * 8)
    F.close()
    ...
    while bv.more_to_read:
        ...
        for round_key in round_key_list:
            ...
            RE = nextRE
        new_content = RE + LE
        new_content.write_to_file(output_file)
    output_file.close()

if __name__ == '__main__':
    start = timeit.timeit()
    DES('image.ppm', 'imageout.ppm', encrypt = True)
    end = timeit.timeit()
    print(end-start)
    #DES('imageout.ppm', 'original.ppm', encrypt = False)
    pass